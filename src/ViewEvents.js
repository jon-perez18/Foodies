...
export const ViewEvents = () => {
  ...
  const mock_data = [
    ...
    },
    {
      ...
      place: "place",
      time: "time",
      attendees: ["Guest1", "Guest2", "Guest3"],
    },
  ]; // temporary, not used
  ...
  return (
    <div className="box">
      <div className="picture">
        <img className="loc-thumbnail" src={logo} alt="location picture" />
      </div>
      <h4>{props.name}</h4>
      <ul>
        <li> {props.hosts} </li>
        <li> {props.time} </li>
        <li> {props.place} </li>
        {props.attendees.length === 0 ? null : (
          <li>{props.attendees.toString()}</li>
        )}
      </ul>
      {props.hosts === props.my_name ? null : (
        <div className="center">
          <button type="button" onClick={() => add_to_attendees(props.my_name)}>
            Request Join
          </button>
        </div>
      )}
